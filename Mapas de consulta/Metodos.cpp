...
void Lista_Poblados::Graficar_Poblaciones(CImg<unsigned char>& img) {
	CImgDisplay ventana;
...
void Lista_Poblados::Parametro_Poblados(struct eslabon_Poblados** Lista, CImg<unsigned char>& imagen) {
	int opcion;
	string elem;
	...
	busquedaBinaria_PobladosHabitantes(Lista, elem,imagen); break;
	default: cout << "Digite el numero correcto" << endl;
	}
}
void Lista_Poblados::busquedaBinaria_PobladosHabitantes(struct eslabon_Poblados** Lista, string elem, CImg<unsigned char>& imagen)
{
	Lista_Poblados aux1;
	mergeSort_PobladosHabitantes(Lista, 0, n - 1);
	int inicio = 0;
	int fin = n - 1;
	int mitad;
	bool continuar = true;
		while ((continuar)) {
			...
			if (stoi(elem) == stoi(Lista[mitad]->Habitantes)) {
				continuar = false;
			}
			else {
				if (stoi(elem) > stoi(Lista[mitad]->Habitantes))
		...
		for (int i = mitad; i < n - 1; ++i) {
			struct eslabon_Poblados* aux = new struct eslabon_Poblados(Lista[i]->NombrePoblacion, Lista[i]->Habitantes, Lista[i]->x, Lista[i]->y);
			aux1 << aux;
			aux1.n++;
		}
		aux1.Mostrar_Poblados();
		aux1.Graficar_Poblaciones(imagen);
		aux1.~Lista_Poblados();
}
//_____________________________________________________________________________________________________________________________________
//_____________________________________________________________________________________________________________________________________
...
void Lista_Escuelas::Graficar_Escuelas(CImg<unsigned char>& img) {
	CImgDisplay ventana;
	CImg<unsigned char> imagen;
	unsigned int w, h;
	w = img.width();
...
void Lista_Escuelas::Graficar_EscuelasParametro(CImg<unsigned char>& img) {
...
void Lista_Escuelas::Parametro_Escuelas(struct eslabon_Escuelas** Lista, CImg<unsigned char>& imagen) {
	int opcion;
	string elem;
	cout << "De que lista quiere aplicar el parametro:  " << endl;
	cout << "             Nombre de Poblados            (1)" << endl;
	...
	case 2:
		cout << "Escriba el numero de habitantes para filtrar: "; cin >> elem;
		busquedaBinaria_EscuelasHabitantes(Lista, elem, imagen); break;
	default: cout << "Digite el numero correcto" << endl;
	}
}
void Lista_Escuelas::busquedaBinaria_EscuelasHabitantes(struct eslabon_Escuelas** Lista, string elem, CImg<unsigned char>& imagen)
{
	Lista_Escuelas aux1;
	mergeSort_EscuelasEstudiantes(Lista, 0, n - 1);
	int inicio = 0;
	int fin = n - 1;
	int mitad;
	bool continuar = true;
		while ((continuar)) {
			...
			if (elem == Lista[mitad]->Estudiantes) {
				continuar = false;
			}
			else {
				if (elem > Lista[mitad]->Estudiantes)
		...
		for (int i = mitad; i < n - 1; ++i) {
			struct eslabon_Escuelas* aux = new struct eslabon_Escuelas(Lista[i]->NombreIntitucion, Lista[i]->Estudiantes, Lista[i]->Provincia, Lista[i]->Canton, Lista[i]->Distrito, Lista[i]->x, Lista[i]->y);
			aux1 << aux;
			aux1.n++;
		}
		aux1.Mostrar_Escuelas();
		aux1.Graficar_Escuelas(imagen);
		aux1.~Lista_Escuelas();
}
void Lista_Escuelas::DosParametro_Escuelas(struct eslabon_Escuelas** Lista, CImg<unsigned char>& imagen) {
	string elem;
	mergeSort_EscuelasEstudiantes(Lista, 0, n - 1);
	...
	cin.ignore();
	getline(cin, provincia);
	DosbusquedaBinaria_EscuelasHabitantes(Lista, elem, imagen, provincia);
}
void Lista_Escuelas::DosbusquedaBinaria_EscuelasHabitantes(struct eslabon_Escuelas** Lista, string elem, CImg<unsigned char>& imagen,string provincia)
{
	Lista_Escuelas aux1;
	mergeSort_EscuelasEstudiantes(Lista, 0, n - 1);
	int inicio = 0;
	...
	for (int i = mitad; i < n - 1; ++i) {
		if(Lista[i]->Provincia == provincia) {
			struct eslabon_Escuelas* aux = new struct eslabon_Escuelas(Lista[i]->NombreIntitucion, Lista[i]->Estudiantes, Lista[i]->Provincia, Lista[i]->Canton, Lista[i]->Distrito, Lista[i]->x, Lista[i]->y);
			aux1 << aux;
			aux1.n++;
		}
...
void Lista_Colegio::Graficar_ColegioParametro(CImg<unsigned char>& img) {
	cout << "Escriba el nombre de la provincia (EN MAYUSCULAS): ";
	string provincia;
	cin.ignore();
	getline(cin, provincia);
	...
	imagen.assign(img);
	const unsigned char negro[] = { 0, 0, 255 };
	struct eslabon_Colegio* auxXs = p_Listas;
	int cont = 0;
	for (int i = 0; i < n; i++) {
		if (auxXs->Provincia == provincia) {
			int auxX = stoi(auxXs->x);
...
void Lista_Colegio::Graficar_Colegio(CImg<unsigned char>& img) {
	CImgDisplay ventana;
	CImg<unsigned char> imagen;
	unsigned int w, h;
	w = img.width();
...
void Lista_Colegio::Parametro_Colegio(struct eslabon_Colegio** Lista, CImg<unsigned char>& imagen) {
	int opcion;
	string elem;
	cout << "De que lista quiere aplicar el parametro:  " << endl;
	cout << "             Nombre de Poblados            (1)" << endl;
	...
	case 2:
		cout << "Escriba el numero de habitantes para filtrar: "; cin >> elem;
		busquedaBinaria_ColegioHabitantes(Lista, elem, imagen); break;
	default: cout << "Digite el numero correcto" << endl;
	}
}
void Lista_Colegio::busquedaBinaria_ColegioHabitantes(struct eslabon_Colegio** Lista, string elem, CImg<unsigned char>& imagen)
{
	Lista_Colegio aux1;
	mergeSort_ColegioEstudiantes(Lista, 0, n - 1);
	int inicio = 0;
	int fin = n - 1;
	int mitad;
	bool continuar = true;
		...
		for (int i = mitad; i < n - 1; ++i) {
			struct eslabon_Colegio* aux = new struct eslabon_Colegio(Lista[i]->NombreIntitucion, Lista[i]->Estudiantes, Lista[i]->Provincia, Lista[i]->Canton, Lista[i]->Distrito, Lista[i]->x, Lista[i]->y);
			aux1 << aux;
			aux1.n++;
		}
		aux1.Mostrar_Colegio();
		aux1.Graficar_Colegio(imagen);
		aux1.~Lista_Colegio();
}
void Lista_Colegio::DosParametro_Colegio(struct eslabon_Colegio** Lista, CImg<unsigned char>& imagen) {
	string elem;
	mergeSort_ColegioEstudiantes(Lista, 0, n - 1);
	cout << "Escriba el numero de habitantes para filtrar: " << endl; cin >> elem;
	cout << "Escriba el nombre de la provincia (EN MAYUSCULAS): ";
	...
	cin.ignore();
	getline(cin, provincia);
	DosbusquedaBinaria_ColegioHabitantes(Lista, elem, imagen, provincia);
}
void Lista_Colegio::DosbusquedaBinaria_ColegioHabitantes(struct eslabon_Colegio** Lista, string elem, CImg<unsigned char>& imagen, string provincia)
{
	Lista_Colegio aux1;
	mergeSort_ColegioEstudiantes(Lista, 0, n - 1);
	int inicio = 0;
